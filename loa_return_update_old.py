...
def _resolve_input_paths() -> tuple[Path, Path]:
    ...
    if sales_path is None:
        ...
        sales_path = _pick_latest(sales_files, "Sales Compensation Report")
    if people_path is None:
        people_files = list(cwd.glob("People*.xls*"))
        people_path = _pick_latest(people_files, "People")
...
def main() -> None:
    ...
    print(f"Using Sales file: {sales_path}")
    print(f"Using People file: {people_path}")
